...
def clean_response(text):
    ...
    for line in lines:
        stripped = line.strip()
        # Skip lines that are just ads/links
        if any(p.lower() in stripped.lower() for p in ad_patterns):
            if len(stripped) < 150 and ("http" in stripped or "powered" in stripped.lower() or stripped.startswith("🌸") or stripped.startswith("---")):
                continue
        cleaned.append(line)
